...
export default function CommandPalette({ isOpen, onClose }: CommandPaletteProps) {
  ...
  const commands = [
    { label: 'Chuyển đến Bảng Điều Khiển Tổng Quan', href: '/', icon: LayoutDashboard },
    { label: 'Mở Duyệt Bài Viết (Bài Chờ Biên Tập)', href: '/inbox', icon: Inbox },
    { label: 'Tạo Bài Viết Mới Trong Soạn Thảo Thông Minh', href: '/composer', icon: PenTool },
    { label: 'Mở Lịch Xuất Bản & Tần Suất Đăng Bài', href: '/calendar', icon: Calendar },
    { label: 'Vận Hành & Đối Soát Xuất Bản (Reconcile)', href: '/reconcile', icon: Send },
    { label: 'Quản Lý Nguồn Quét Tin Báo', href: '/sources', icon: Globe },
    { label: 'Xem Báo Cáo Phân Tích Hiệu Suất & Chi Phí', href: '/analytics', icon: BarChart3 },
    { label: 'Cấu Hình Hệ Thống & Bảo Mật Fanpage', href: '/settings', icon: Settings },
  ];
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        <div className="p-3.5 border-b border-slate-200 flex items-center space-x-3 bg-slate-50">
          ...
          <input
            type="text"
            autoFocus
            placeholder="Tìm kiếm tính năng hoặc gõ lệnh... (Ctrl+K)"
            value={query}
        ...
        <div className="max-h-80 overflow-y-auto p-2">
          {filtered.length === 0 ? (
            <p className="p-4 text-xs text-slate-500 text-center">Không tìm thấy lệnh nào phù hợp với "{query}"</p>
          ) : (
            filtered.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[10px] text-slate-400 font-mono">Chuyển</span>
                </button>
              );
            })
          )}
        ...
        <div className="p-2.5 bg-slate-50 border-t border-slate-200 text-[11px] text-slate-500 flex items-center justify-between font-mono">
          <span>
            Nhấn <kbd className="px-1.5 py-0.5 bg-white border border-slate-300 rounded shadow-2xs font-sans">Esc</kbd> để đóng
          </span>
